...
Connection.prototype.connect = function() {
  ...
  return new Promise((resolve, reject) => {
    const clear = () => {
      self.off('ready', resolve);
      self.off('error', reject);
    };
    self.on('ready', () => {
      clear();
      resolve();
...
Connection.prototype.end = function() {
  var self = this;
  return this._enqueue('LOGOUT', function() {
    self._queue = [];
    self._curReq = undefined;
    self._sock.end();
  });
...
Connection.prototype.openBox = function(name, readOnly) {
...
Connection.prototype.closeBox = function(shouldExpunge) {
  ...
  if (shouldExpunge) {
    return this._enqueue('CLOSE', function(err) {
      if (!err)
        self._box = undefined;
    });
  } else {
    if (this.serverSupports('UNSELECT')) {
      ...
      // alternative "hack"
      return this._enqueue('UNSELECT', function(err) {
        if (!err)
          self._box = undefined;
      });
    } else {
      ...
      return this._enqueue('SELECT "' + badbox + '"', function(err) {
        self._box = undefined;
      });
    }
  }
...
Connection.prototype.renameBox = function(oldname, newname) {
  var encoldname = escape(utf7.encode(''+oldname)),
      encnewname = escape(utf7.encode(''+newname)),
      self = this;

  return this._enqueue('RENAME "' + encoldname + '" "' + encnewname + '"',
    function(err) {
      if (err) return;
...
Connection.prototype._move = async function(which, uids, boxTo) {
  ...
  if (this.serverSupports('MOVE')) {
    ...
    return this._copy(which, uids, boxTo).then(function ccb(err, info) {
      if (err)
        return cb(err, info);
...
Connection.prototype.setQuota = function(quotaRoot, limits = {}) {
  var triplets = '';
  for (var l in limits) {
    if (triplets)
      triplets += ' ';
  ...
  return this._enqueue('SETQUOTA "' + quotaRoot + '" (' + triplets + ')',
    (err, quotalist) => {
      if (err) return;
...
Connection.prototype.getQuota = function(quotaRoot) {
  quotaRoot = escape(utf7.encode(''+quotaRoot));

  this._enqueue('GETQUOTA "' + quotaRoot + '"', (err, quotalist) => {
    if (err) return;
    return quotalist[0];
  });
};

Connection.prototype.getQuotaRoot = function(boxName) {
  boxName = escape(utf7.encode(''+boxName));

  this._enqueue('GETQUOTAROOT "' + boxName + '"', (err, quotalist) => {
    if (err) return;
...
Connection.prototype._enqueue = function(fullcmd, promote, cb) {
  ...
  return info.promise = new Promise((resolve, reject) => {
    ...
    info.cb = function (err, result) {
      if (cb) result = cb.apply(this, arguments) || result;
