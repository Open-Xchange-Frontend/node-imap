...
Connection.prototype.connect = async function () {
  ...
  if (config.tls) {
    ...
    for (const k in config.tlsOptions) {
  ...
  return Promise.race([
    this._login(),
    new Promise((resolve, reject) => setTimeout(reject, config.connTimeout))
  ]).catch(() => {
...
Connection.prototype.openBox = async function (name, { readOnly = false, qresync }) {
  if (this.state !== 'authenticated') {
    throw new Error('Not authenticated')
  }
  if (qresync && !this._qreSyncEnabled) {
    throw new Error('QRESYNC was not enabled')
  }
  ...
  if (qresync) {
    const params = [qresync.uidvalidity, qresync.modseq]
    if (qresync.knownUIDs) {
      let uids = qresync.knownUIDs
    ...
    if (qresync.knowSequenceSet && qresync.knownUIDSet) {
      params.push(`(${qresync.knowSequenceSet.join(',')} ${qresync.knownUIDSet.join(',')})`)
    }
    cmd += ` (QRESYNC (${params.join(' ')}))`
  } else if (this.serverSupports('CONDSTORE')) {
    cmd += ' (CONDSTORE)'
  }
...
Connection.prototype._fetch = function (which, uids, options) {
  ...
  if (this.serverSupports('CONDSTORE') && !this._box.nomodseq && !this._qreSyncEnabled) {
  ...
  for (i = 0, len = modkeys.length, key; i < len; ++i) {
    ...
    if (key === 'CHANGEDSINCE' && (this.serverSupports('CONDSTORE') || this._qreSyncEnabled) && !this._box.nomodseq) {
      modstr += key + ' ' + modifiers[modkeys[i]] + ' '
      if (this._qreSyncEnabled) modstr += 'VANISHED '
    }
  }
  if (modstr.length > 2) {
  ...
  emitter.all = async function () {
    return new Promise((resolve, reject) => {
      this.chunks(Infinity).once('chunk', resolve)
      this.once('error', reject)
    })
  }
  emitter.chunks = function (chunkSize) {
    let receiveCount = 0
    let emitCount = 0
    let value = []
    let ended = false
    this.on('message', (e, seqno) => {
      const data = {}
      receiveCount++
      e.on('body', (stream, info) => {
        const buffer = Buffer.alloc(info.size)
        let offset = 0
        stream.on('data', (chunk) => {
          buffer.write(chunk.toString('utf8'), offset)
          offset += chunk.length
        })
        stream.once('end', () => {
          if (info.which && info.which.indexOf('HEADER') === 0) {
            const header = parseHeader(buffer.toString('utf8'))
            data.header = data.header || {}
            Object.assign(data.header, header)
          } else {
            data[info.which] = buffer.toString('utf8')
          }
        })
      })
      e.once('attributes', (attrs) => {
      ...
      e.once('end', () => {
        setImmediate(() => {
          emitCount++
          value.push(data)
          const done = ended && emitCount === receiveCount
          if (value.length % chunkSize === 0 || done) {
  ...
  return (req.bodyEmitter = emitter)
}
...
Connection.prototype.getMetadata = function (keys, mailbox = '', depth) {
  if (!this.serverSupports('METADATA-SERVER') && !this.serverSupports('METADATA')) {
    throw new Error('Server does not support METADATA')
  }
  if (mailbox && !this.serverSupports('METADATA')) {
    throw new Error('Server does not support METADATA')
  }
  ...
  return this._enqueue(`GETMETADATA ${depth}"${mailbox}" ${keys}`)
}
...
Connection.prototype._resUntagged = function (info) {
  ...
  let i, len, box, attrs, key, cbargs
  ...
  if (type === 'bye') {
    ...
    if (this._box) {
      const prev = this._box.messages.total
      const now = info.num
    ...
    if (!info.text.earlier) {
      // decrement messages.total for every uid in the vanished command
      const ranges = info.text.uids.map(uid => {
        if (uid.indexOf(':') < 0) return [parseInt(uid, 10)]
        return uid.split(':').map(uid => parseInt(uid, 10))
      })
      const amount = ranges.reduce((memo, range) => {
        if (range.length === 1) return memo + 1
        return memo + (range[1] - range[0]) + 1
      }, 0)
      this._box.messages.total -= amount
    }
    this.emit('vanished', info.text.uids, !!info.text.earlier)
  } else if (type === 'ok') {
    if (this.state === 'connected' && !this._curReq) {
      this.emit('connected')
...
Connection.prototype._login = async function () {
  try {
    ...
    if (this._config.qresync && this.serverSupports('QRESYNC') && this.serverSupports('ENABLE')) {
      await this._enqueue('ENABLE QRESYNC')
      this._qreSyncEnabled = true
    }
...
Connection.prototype._starttls = async function () {
  ...
  // Host name may be overridden the tlsOptions
  for (const k in this._config.tlsOptions) {
...
Connection.prototype._enqueue = function (fullcmd, promote = false, map) {
  const info = {
  ...
  info.promise = new Promise((resolve, reject) => {
    info.cb = async function (err, result) {
