...
const RE_LITERAL = /\{(\d+)\}$/
const RE_UNTAGGED = /^\* (?:(OK|NO|BAD|BYE|FLAGS|ID|LIST|XLIST|LSUB|SEARCH|STATUS|CAPABILITY|NAMESPACE|PREAUTH|SORT|THREAD|ESEARCH|QUOTA|QUOTAROOT|METADATA|VANISHED)|(\d+) (EXPUNGE|FETCH|RECENT|EXISTS))(?:(?: \[([^\]]+)\])?(?: (.+))?)?$/i
...
Parser.prototype._resUntagged = function () {
  ...
  if ((m = RE_BODYLITERAL.exec(this._buffer))) {
    ...
    const type = (m[1] || m[3]).toLowerCase()
    ...
    if (type === 'flags' ||
      ...
      val = parseMetadata(m[5], this._literals)
    } else if (type === 'vanished') {
      val = parseVanished(m[5], this._literals)
    } else {
      val = m[5]
    }
...
function parseMetadata (text, literals) {
  const r = parseExpr(text, literals)
  const result = {
    mailbox: r[0],
    data: {}
  }
  const pairs = [].concat(r[1])

  for (let i = 0; i < pairs.length; i += 2) {
    result.data[pairs[i]] = pairs[i + 1]
  }
...
function parseVanished (text, literals) {
  const list = parseExpr(text, literals)
  const attrs = { uids: (list[1] || list[0]).split(',') }
  if (list.length === 2) attrs.earlier = list[0].length === 1 && list[0][0] === 'EARLIER'
  return attrs
}
